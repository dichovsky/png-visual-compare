...
- opens the target with `O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW`, falling back to a reopen without `O_CREAT` on `EEXIST` (target-component symlink race closed by SECU-03; a symlink reports `EEXIST` under `O_EXCL` and then surfaces as `ELOOP` on the reopen).
- closes the handle on failure without unlinking the target path. Even after an `O_EXCL` create or a successful identity check, another writer can replace that path before cleanup. Node has no atomic unlink-by-inode operation, so preserving replacement files takes precedence over removing an empty or partial output left by a failed write.
